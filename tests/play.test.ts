...
describe('Gra lokalna 1 na 1', () => {
  ...
  test('odrzuca podwójne kliknięcie, nielegalny indeks i błędny wybór talii', () => {
    ...
    const end = view.actions.find(e => e.action.type === 'endTurn')!;
    session.act(end.id, view.revision);
    expect(() => session.act(end.id, view.revision)).toThrow('nieaktualny');
...
test('HTTP: dwa urządzenia, prywatne ręce, tury, osobne pokoje i ochrona przed obcą stroną', async () => {
  ...
  expect((await post('/api/action', { id: end.id, revision: activeView.revision }, waiting.token)).status).toBe(403);
  expect((await post('/api/action', { id: end.id, revision: activeView.revision }, undefined)).status).toBe(401);
  expect((await post('/api/action', { id: end.id, revision: activeView.revision }, active.token, 'https://example.com')).status).toBe(403);
  const next = await (await post('/api/action', { id: end.id, revision: activeView.revision }, active.token)).json();
  expect(next.phase).toBe('waiting-for-turn');
  expect(next.observation.self.hand).toHaveLength(6);
  ...
  expect((await post('/api/action', { id: end.id, revision: activeView.revision }, waiting.token)).status).toBe(409);
