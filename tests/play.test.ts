...
test('HTTP: dwa urządzenia, prywatne ręce, tury, osobne pokoje i ochrona przed obcą stroną', async () => {
  ...
  expect(active.view.actions.map((e: { action: { type: string } }) => e.action.type)).not.toContain('createEnergy');
  ...
  expect(otherTurn.observation.self.energy).toBe(1);
  expect(otherTurn.observation.self.maxEnergy).toBe(1);
  expect((await post('/api/action', { id: end.id, revision: active.view.revision }, waiting.token)).status).toBe(409);
  const secondEnd = otherTurn.actions.find((e: { action: { type: string } }) => e.action.type === 'endTurn');
  await post('/api/action', { id: secondEnd.id, revision: otherTurn.revision }, waiting.token);
  const roundTwo = await (await get(active.token)).json();
  ...
  expect(roundTwo.observation.opponent.energy).toBe(1);
